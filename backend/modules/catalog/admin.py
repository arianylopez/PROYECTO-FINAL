from django.contrib import admin
from django.utils.html import format_html
from django.forms.models import model_to_dict
from django.utils.timezone import now
from .models import Genre, Movie, Room, Screening, AuditLog

def create_audit_log(request, action, obj, snapshot_before=None):
    snapshot_after = model_to_dict(obj) if action != 'DELETE' else None
    
    if snapshot_after and 'genres' in snapshot_after:
        snapshot_after['genres'] = list(obj.genres.values_list('name', flat=True))
    if snapshot_before and 'genres' in snapshot_before:
        pass 
        
    AuditLog.objects.create(
        admin_user=request.user,
        action=action,
        table_name=obj._meta.model_name,
        record_id=str(obj.pk),
        snapshot_before=snapshot_before,
        snapshot_after=snapshot_after
    )

@admin.register(Movie)
class MovieAdmin(admin.ModelAdmin):
    list_display = ('poster_preview', 'title', 'release_date', 'rating_classification', 'is_active')
    list_filter = ('is_active', 'rating_classification', 'genres')
    search_fields = ('title', 'director')
    filter_horizontal = ('genres',) 
    
    actions = ['soft_delete_movies']

    def poster_preview(self, obj):
        if obj.poster_url:
            return format_html('<img src="{}" style="width: 50px; height: 75px; object-fit: cover; border-radius: 4px;" />', obj.poster_url)
        return "No Image"
    poster_preview.short_description = 'Afiche'

    def save_model(self, request, obj, form, change):
        snapshot_before = None
        if change:
            old_obj = Movie.objects.get(pk=obj.pk)
            snapshot_before = model_to_dict(old_obj)
            
        super().save_model(request, obj, form, change)
        
        create_audit_log(request, 'UPDATE' if change else 'CREATE', obj, snapshot_before)

    @admin.action(description='Dar de baja películas seleccionadas (Soft Delete + Cascading)')
    def soft_delete_movies(self, request, queryset):
        count = 0
        for movie in queryset:
            if movie.is_active:
                movie.is_active = False
                movie.save()
                
                future_screenings = movie.screenings.filter(start_time__gte=now(), is_active=True)
                screenings_count = future_screenings.update(is_active=False)
                
                create_audit_log(request, 'DELETE', movie, model_to_dict(movie))
                count += 1
                
        self.message_user(request, f'{count} películas dadas de baja. Funciones futuras canceladas.')

@admin.register(AuditLog)
class AuditLogAdmin(admin.ModelAdmin):
    list_display = ('action', 'table_name', 'admin_user', 'timestamp')
    list_filter = ('action', 'table_name', 'timestamp')
    search_fields = ('admin_user__username', 'record_id')
    
    def has_add_permission(self, request): return False
    def has_change_permission(self, request, obj=None): return False
    def has_delete_permission(self, request, obj=None): return False

admin.site.register(Genre)
admin.site.register(Room)
admin.site.register(Screening)