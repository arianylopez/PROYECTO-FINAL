  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      processSuccessfulLogin(
        ...
        response.data.user?.name || data.email.split('@')[0],
        data.email
      );
    } catch (error: any) {
    ...
    <div className="auth-layout">
      <div className="auth-layout__sidebar">
        <form className="register-form" onSubmit={handleSubmit(onSubmit)}>
          ...
          <Link to="/forgot-password" className="register-form__link register-form__link--forgot">
          ...
          <div className="register-form__footer">
            ...
            <Link to="/register" className="register-form__link">
              Sign Up
            </Link>
          </div>
        </form>
      ...
      {devicesLimit.length > 0 && (
        <div className="auth-modal">
          <div className="auth-modal__content">
            ...
            <p className="auth-modal__desc auth-modal__desc--muted">
              Revoca una sesión activa para ingresar desde este nuevo dispositivo.
            </p>
            {devicesLimit.map((d) => (
              <div key={d.id} className="auth-modal__device">
                <div>
                  <strong className="auth-modal__device-name">{d.name}</strong>
                  <span className="auth-modal__device-date">
                    Última vez: {new Date(d.last_seen).toLocaleDateString()}
                  </span>
                </div>
                <button type="button" onClick={() => handleRevoke(d.id)} className="auth-modal__btn-revoke">
                  Revocar
                </button>
              </div>
            ))}
            <button type="button" onClick={() => setDevicesLimit([])} className="auth-modal__btn-cancel">
              Cancelar
            </button>
          </div>
        </div>
