      ...
      {recs && recs.items.length > 0 && (
        <section className="recs">
          <div className="recs__header">
            <div>
              <h2 className="recs__title">{recs.title}</h2>
              {recs.subtitle && <p className="recs__subtitle">{recs.subtitle}</p>}
            </div>

            <div className="recs__controls">
              <button onClick={() => scrollCarousel(-280)} className="recs__control-btn">
                <svg
                  width="22"
                  height="22"
                  viewBox="0 0 24 24"
          ...
          <div ref={carouselRef} className="recs__carousel">
            {recs.items.map((item) => {
              ...
              return (
                <div
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') navigate(`/movie/${item.id}`);
                  }}
                  role="button"
                  tabIndex={0}
                ...
                >
                  <div className="recs__card-image-box">
                    {user && (
                      <button onClick={(e) => handleDismiss(e, item.id)} className="recs__card-dismiss-btn">
                        ✕
                      </button>
                    )}

                    <img src={item.poster_url} alt={item.title} className="recs__card-img" />
                  </div>

                  <div className="recs__card-panel">
                    <span
                      className={`recs__card-reason-short ${recs.is_personalized ? 'recs__card-reason-short--personalized' : 'recs__card-reason-short--generic'}`}
                    >
                      {getShortReason(item.reason)}
                    </span>
                    <p className="recs__card-reason-long">{item.reason}</p>
                  </div>
                </div>
              );
            })}
