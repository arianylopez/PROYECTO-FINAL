    ...
    <div className="review-card">
      <div className="review-card__header">
        <div className="review-card__user-info">
          <div className="review-card__avatar">{review.user_name.charAt(0).toUpperCase()}</div>
...
export const MovieDetailPage = () => {
  ...
  useEffect(() => {
    if (user && id) {
      fetchWatchlistStatus(id, user.id)
        .then((status) => setIsWatchlisted(status))
        .catch(console.error);
    }
  }, [user, id]);
        ...
        <div className="movie-detail__hero-content">
          ...
          <div className="movie-detail__info">
            ...
            <div className="movie-detail__actions">
              <button onClick={handleBuyTicket} className="movie-detail__btn">
                Comprar entradas
              </button>
        ...
        <div className="reviews-section__container">
          <div className="reviews-section__summary">
            <div className="reviews-section__stats">
              <div className="reviews-section__stats-score">{reviewsData?.stats.avg_score.toFixed(1) || '0.0'}</div>
              <div className="reviews-section__stats-stars">
                {[1, 2, 3, 4, 5].map((i) => (
                  <svg
                    ...
                    height="22"
                    viewBox="0 0 24 24"
                    fill={i <= Math.round(reviewsData?.stats.avg_score || 0) ? 'currentColor' : 'none'}
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
              ...
              <div className="reviews-section__stats-label">{getSentimentLabel(reviewsData?.stats.avg_score || 0)}</div>
              <div className="reviews-section__stats-total">
                Basado en {reviewsData?.stats.total_ratings || 0} calificaciones
              </div>
            </div>
            ...
            <div className="reviews-section__bars">
              <div className="reviews-section__bar-row">
                ...
                <div className="reviews-section__bar-track">
                  <div
                    className="reviews-section__bar-fill reviews-section__bar-fill--pos"
                    style={{ width: `${posPct}%` }}
                  ></div>
                </div>
              ...
              <div className="reviews-section__bar-row">
                ...
                <div className="reviews-section__bar-track">
                  <div
                    className="reviews-section__bar-fill reviews-section__bar-fill--neu"
                    style={{ width: `${neuPct}%` }}
                  ></div>
                </div>
              ...
              <div className="reviews-section__bar-row">
                ...
                <div className="reviews-section__bar-track">
                  <div
                    className="reviews-section__bar-fill reviews-section__bar-fill--neg"
                    style={{ width: `${negPct}%` }}
                  ></div>
                </div>
            ...
            {!user ? (
              <div className="reviews-section__form-login">
                ...
                <button
                  ...
                  className="reviews-section__form-btn"
                >
                  Iniciar Sesión
                </button>
              </div>
              ...
              <form onSubmit={handleReviewSubmit} className="reviews-section__form">
                {errorMsg && <div className="reviews-section__form-error">⚠️ {errorMsg}</div>}

                <div className="reviews-section__form-group">
                  <p className="reviews-section__form-label">
                ...
                <div className="reviews-section__form-group">
                  <label className="reviews-section__form-label" htmlFor="reviewText">
                    Tu Reseña <span className="optional">(Opcional)</span>
                  </label>
                  <textarea
                    id="reviewText"
                    value={reviewText}
                  ...
                  />
                  <div className="reviews-section__form-count">{reviewText.length} / 500</div>
                </div>
          ...
          <div>
            <div className="reviews-section__filters">
              <button
                ...
                className={`reviews-section__filter-btn ${filter === 'ALL' ? 'reviews-section__filter-btn--active' : ''}`}
              >
                Todas ({reviewsData?.reviews.length || 0})
              </button>
              <button
                onClick={() => setFilter('POSITIVE')}
                className={`reviews-section__filter-btn ${filter === 'POSITIVE' ? 'reviews-section__filter-btn--active' : ''}`}
              >
                Positivas ({reviewsData?.stats.positive || 0})
              </button>
              <button
                onClick={() => setFilter('NEUTRAL')}
                className={`reviews-section__filter-btn ${filter === 'NEUTRAL' ? 'reviews-section__filter-btn--active' : ''}`}
              >
                Neutrales ({reviewsData?.stats.neutral || 0})
              </button>
              <button
                onClick={() => setFilter('NEGATIVE')}
                className={`reviews-section__filter-btn ${filter === 'NEGATIVE' ? 'reviews-section__filter-btn--active' : ''}`}
              >
                Negativas ({reviewsData?.stats.negative || 0})
              </button>
            </div>

            {filteredReviews.length === 0 ? (
              <div className="reviews-section__empty">No hay reseñas escritas en esta categoría.</div>
            ) : (
