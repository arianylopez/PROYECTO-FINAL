      ...
      <div className="profile__container">
        <div className="profile__sidebar">
          <div className="profile__user">
            <div className="profile__avatar">{user?.name?.charAt(0).toUpperCase() || 'U'}</div>
            <div>
              ...
              {activeTab === 'tickets' &&
                (orders.length === 0 ? (
                  <div className="profile__empty">
                    ...
                    <button onClick={() => navigate('/home')} className="profile__empty-btn">
                      Explorar Cartelera
                    </button>
                  </div>
                ) : (
                          ...
                          <div className="tickets__list">
                            {upcomingOrders.map((order) => (
                              <div
                                ...
                                role="button"
                                tabIndex={0}
                                className={`ticket-card ${selectedOrder?.id === order.id ? 'ticket-card--active' : ''}`}
                              >
                                ...
                                <div className="ticket-card__info">
                                  ...
                                  <p className="ticket-card__text">
                                    {new Date(order.start_time).toLocaleString('es-ES', {
                                      weekday: 'short',
                                      day: 'numeric',
                                      month: 'short',
                                      hour: '2-digit',
                                  ...
                                  <p className="ticket-card__text ticket-card__text--muted">
                                    {order.room_name} • Asientos: {order.seat_labels.join(', ')}
                                  </p>
                                </div>
                              </div>
                            ))}
                          ...
                          <div className="tickets__list">
                            {pastOrders.map((order) => (
                              <div
                                ...
                                role="button"
                                tabIndex={0}
                                className={`ticket-card ticket-card--past ${selectedOrder?.id === order.id ? 'ticket-card--active' : ''}`}
                              >
                                <img
                                  src={order.poster_url}
                                  className="ticket-card__poster ticket-card__poster--past"
                                  alt="Poster"
                                />
                                <div className="ticket-card__info">
                                  <div className="ticket-card__header">
                                  ...
                                  <p className="ticket-card__text ticket-card__text--muted">
                                    {new Date(order.start_time).toLocaleDateString('es-ES', {
                                      day: 'numeric',
                                      month: 'long',
                                      year: 'numeric',
                                    })}
                    ...
                    {selectedOrder && (
                      <div className="ticket-detail">
                        <div className="ticket-detail__hero">
                          <div
                            className="ticket-detail__hero-bg"
                            style={{ backgroundImage: `url(${selectedOrder.poster_url})` }}
                          ></div>
                          <button onClick={() => setSelectedOrder(null)} className="ticket-detail__close">
                            ✕
                          </button>
                        ...
                        <div className="ticket-detail__body">
                          <div className="ticket-detail__qr">
                            <img
                              src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${selectedOrder.id}`}
                              alt="QR"
                          ...
                          <div className="ticket-detail__section">
                            ...
                            <div className="ticket-detail__grid">
                              <div>
                                ...
                                {new Date(selectedOrder.start_time).toLocaleDateString('es-ES')}
                              </div>
                              <div>
                                <span className="ticket-detail__label">Hora</span>{' '}
                                {new Date(selectedOrder.start_time).toLocaleTimeString('es-ES', {
                                  hour: '2-digit',
                                  minute: '2-digit',
                                })}
                              </div>
                              <div>
                                <span className="ticket-detail__label">Sala</span> {selectedOrder.room_name}
                              </div>
                              <div>
                                <span className="ticket-detail__label">
                                  Asientos ({selectedOrder.seat_labels.length})
                                </span>{' '}
                                <strong className="ticket-detail__highlight">
                                  {selectedOrder.seat_labels.join(', ')}
                                </strong>
                              </div>
                            </div>
                          </div>
                          <div className="ticket-detail__summary">
                            <h4 className="ticket-detail__section-title ticket-detail__section-title--muted">
                              Resumen de Pago
                            </h4>
                            <div className="ticket-detail__row">
                              <span>Estado</span>{' '}
                              <span className="ticket-detail__status-text">{selectedOrder.status}</span>
                            </div>
                            <div className="ticket-detail__row">
                              <span>Orden</span> <span className="ticket-detail__id-text">{selectedOrder.id}</span>
                            </div>
                    ...
                    <div className="watchlist-grid">
                      {watchlist.map((item) => (
                        <div
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'Enter' || e.key === ' ') navigate(`/movie/${item.movie_id}`);
                          }}
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <div className="watchlist-card__body">
                            <h3 className="watchlist-card__title">{item.movie_title}</h3>
                          </div>
                          <button
                            onClick={(e) => handleRemoveFromWatchlist(item.movie_id, e)}
                        ...
                        <div key={act.id} className="activity-item">
                          ...
                          <div className="activity-item__card">
                            <div className="activity-item__header">
                              <strong className="activity-item__title" style={{ color: getIconColor(act.type) }}>
                                {act.title}
                              </strong>
