...
export const ScreeningSelectionPage = () => {
  ...
  useEffect(() => {
    const loadSelectionData = async () => {
      try {
        if (!id) return;
        const [screeningsResult, upcomingResult] = await Promise.all([fetchMovieScreenings(id), fetchMovies(1, 12)]);
  ...
  const groupedByDate = useMemo(() => {
    ...
    return data.screenings.reduce(
      (acc, sc) => {
        ...
        if (isToday)
          dateKey = `Hoy ${dateObj.getDate()} ${dateObj.toLocaleDateString('es-ES', { month: 'short' })}`.toUpperCase();
        else if (isTomorrow)
          dateKey =
            `Mañ. ${dateObj.getDate()} ${dateObj.toLocaleDateString('es-ES', { month: 'short' })}`.toUpperCase();
  ...
  const roomsFiltered = useMemo(() => {
    ...
    const filtered =
      selectedFormat === 'Todos' ? dayScreenings : dayScreenings.filter((s) => s.format === selectedFormat);
  ...
  return (
    <div className="screening-page">
      ...
      <div className="date-slider">
        <button onClick={() => handleScrollSlider('L')} className="date-slider__nav">
          ‹
        </button>
        ...
        <button onClick={() => handleScrollSlider('R')} className="date-slider__nav">
          ›
        </button>
      </div>
      ...
      <div className="rooms-list">
        {Object.keys(roomsFiltered).length === 0 ? (
          <div className="rooms-list__empty">
            <p className="rooms-list__empty-text">
              No hay funciones que coincidan con los filtros seleccionados para este día.
            </p>
          </div>
        ) : (
          Object.entries(roomsFiltered).map(([roomName, screenings]) => (
            <div key={roomName} className="room-card">
              ...
              <div className="room-card__times">
                {screenings.map((sc) => (
                  <button key={sc.id} onClick={() => handleOpenTicketModal(sc)} className="time-btn">
      ...
      <TicketSelectionModal
        ...
        screeningTime={
          activeScreening
            ? new Date(activeScreening.start_time).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })
            : ''
        }
