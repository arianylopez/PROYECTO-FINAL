...
export const SeatSelectionPage = () => {
  ...
  const handleLockSeats = async () => {
    try {
      ...
      const res = await lockScreeningSeats(
        id,
      ...
      <div className="seat-page__content">
        {hasActiveReservation && (
          <div className="reservation-alert">
            <div>
              ...
              <p className="reservation-alert__desc">
                El sistema ha recuperado tus asientos. Completa el pago antes de que expire la reserva.
              </p>
            </div>
        ...
        <div className="layout-grid">
          ...
          <div className="seat-map-section">
            <div className="seat-map-section__screen">PANTALLA</div>
                    ...
                    {data.seats
                      ...
                      .map((seat) => {
                        if (seat.type === 'corridor')
                          return <div key={seat.id} className="seat-item seat-item--corridor" />;
                        ...
                        return (
                          <div
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'Enter' || e.key === ' ') toggleSeat(seat);
                            }}
                            role="button"
                            tabIndex={0}
                            title={`${row}-${seat.col}`}
            ...
            <div className="seat-legend">
              <div className="seat-legend__item">
                <div className="seat-legend__box seat-legend__box--available"></div> Disponible
              </div>
              <div className="seat-legend__item">
                <div className="seat-legend__box seat-legend__box--locked"></div> En proceso (Otro)
              </div>
              <div className="seat-legend__item">
                <div className="seat-legend__box seat-legend__box--selected"></div> Tu Reserva
              </div>
              <div className="seat-legend__item">
                <div className="seat-legend__box seat-legend__box--sold"></div> Ocupado
              </div>
            </div>
          </div>

          <div className="booking-summary">
            {errorMsg && <div className="booking-summary__error">{errorMsg}</div>}
              ...
              {purchaseSummary.items.map((item, idx) => (
                <div key={idx} className="booking-summary__item">
                  <span className="booking-summary__item-name">
                    {item.name} <span className="booking-summary__item-qty">x{item.qty}</span>
                  </span>
              ...
              {selectedSeats.length > 0 ? (
                selectedSeats.map((s) => (
                  <span key={s.id} className="booking-summary__seat-badge">
                    {s.row}
                    {s.col}
                    {selectedSeats[selectedSeats.length - 1].id !== s.id ? ',' : ''}
                  </span>
                ))
              ) : (
            ...
            <div className="booking-summary__actions">
              {hasActiveReservation ? (
                <button onClick={handleContinuePayment} className="btn-primary">
                  Retomar Pago
                </button>
              ) : (
                <button
