              ...
              <div className="ticket-visual__body">
                ...
                <div className="ticket-visual__grid">
                  <div>
                    ...
                    <strong className="ticket-visual__val">{screenDate.toLocaleDateString('es-ES', { day:'2-digit', month:'short', year: 'numeric' })}</strong>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <span className="ticket-visual__label">Hora</span>
                    <strong className="ticket-visual__val">{screenDate.toLocaleTimeString('es-ES', { hour:'2-digit', minute:'2-digit' })}</strong>
                  </div>
                </div>
          ...
          <div className="ticket-receipt">
            ...
            <div className="ticket-receipt__section">
              ...
              <div className="ticket-receipt__grid">
                <div><span className="ticket-receipt__label">Nombre</span> {user?.name || 'Usuario CinemaPlus'}</div>
                <div><span className="ticket-receipt__label">Email</span> {user?.email || 'usuario@correo.com'}</div>
              </div>
            </div>
            ...
            <div className="ticket-receipt__section">
              ...
              <ul className="ticket-receipt__list">
                <li><strong className="ticket-receipt__list-label">Película:</strong> {data.movie.title}</li>
                <li><strong className="ticket-receipt__list-label">Asientos:</strong> {seatsLabels.join(', ')}</li>
                <li><strong className="ticket-receipt__list-label">Entradas:</strong> {invoice.tickets.map((t:any) => `${t.name} (x${t.qty})`).join(' • ')}</li>
              </ul>
            </div>
            ...
            <div className="ticket-receipt__breakdown">
              ...
              <div className="ticket-receipt__totals">
                <div className="ticket-receipt__row"><span>Subtotal</span><span>Bs. {invoice.subtotal.toFixed(2)}</span></div>
                <div className="ticket-receipt__row"><span>Comisión de servicio</span><span>Bs. {invoice.fee.toFixed(2)}</span></div>
                <div className="ticket-receipt__row"><span>Impuestos de ley</span><span>Bs. {invoice.tax.toFixed(2)}</span></div>
                <div className="ticket-receipt__row"><span>Método de Pago</span><span className="ticket-receipt__val-method">{res.method}</span></div>
              </div>
